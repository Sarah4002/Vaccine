          ...
          <div className="sup-card">
            ...
            {loading ? (
              ...
              <div style={{ display: 'grid', gap: '12px' }}>
                {tickets.map(ticket => (
                  <div key={ticket.id} className="sig-commune-item" style={{ cursor: 'default', background: 'white' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                      ...
                      <span style={{ fontSize: '10px', color: 'var(--text-muted)' }}>{new Date(ticket.createdAt).toLocaleString()}</span>
                    </div>
                    <div style={{ fontWeight: 700 }}>{ticket.titre}</div>
                    <div style={{ fontSize: '12px', opacity: 0.7 }}>ID: {ticket.id.substring(0, 8)}</div>
                  </div>
                ))}
              </div>
            )}
