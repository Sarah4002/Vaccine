...
const EMPTY_STATS = {
  ...
  parGroupeSanguin: [],
  vaccinationsParMois: [],
  vaccinationsParVaccin: [],
  vaccinationsParType: [],
  vaccinationsParStatut: [],
  vaccinationsParGrade: [],
  vaccinationsParSexe: [],
};
...
export default function Dashboard({ setPage }) {
  ...
  const loadDashboardData = async (activeFilters = {}) => {
    setDashboardError('');
    const [statsResult, rappelsResult] = await Promise.allSettled([
    ...
    if (statsResult.status === 'fulfilled') {
      ...
      nextError = 'Impossible de charger les statistiques du tableau de bord.';
    }

    if (rappelsResult.status === 'fulfilled') {
      setRappels(normalizeRappels(rappelsResult.value));
    } else {
      nextError = nextError
        ? `${nextError} Les rappels sont indisponibles.`
  ...
  useEffect(() => {
    ...
    api.getStats(filters)
      ...
      .catch(() => {
        setDashboardError('Impossible de rafraîchir les statistiques.');
      })
      .finally(() => {
  ...
  const moisLabels = (dashboardStats.vaccinationsParMois || []).map(d => {
  ...
  const sexeData     = (dashboardStats.parSexe || []).map(s => ({ name: s.sexe === 'M' ? 'Hommes' : s.sexe === 'F' ? 'Femmes' : s.sexe, value: s.count }));
  const topVaccins   = (dashboardStats.vaccinationsParVaccin || []).slice(0, 6);
  const maxVaccin    = topVaccins[0]?.count || 1;
  const topWilayas   = (dashboardStats.parWilaya || []).slice(0, 6);
  const maxWilaya    = topWilayas[0]?.count || 1;
  const topFonctions = (dashboardStats.parFonction || []).slice(0, 5);
  const maxFonction  = topFonctions[0]?.count || 1;
  const ageData      = dashboardStats.parAge || [];
  const typeData     = (dashboardStats.vaccinationsParType || []).slice(0, 5);
  const statutData   = dashboardStats.vaccinationsParStatut || [];
  const gradeData    = dashboardStats.vaccinationsParGrade || [];
    ...
    <div>
      ...
      {dashboardError && (
        <div className="alert alert-warning" style={{ marginBottom: 20 }}>
          <div>
            <div style={{ fontWeight: 800, marginBottom: 4 }}>Chargement partiel</div>
      ...
      <div className="stats-grid" style={{ gap: 16 }}>
        {[
          { icon: <><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" /><circle cx="12" cy="7" r="4" /></>, label: 'Total Patients',     val: dashboardStats.totalPatients,         page: 'patients' },
          { icon: <><path d="M12 2l-2 2 4 4 2-2-4-4z" /><path d="M10 4L2 12l2 2-2 2 2 2 2-2 2 2 8-8-8-8z" /><line x1="14" y1="10" x2="4" y2="20" /></>,  label: 'Total Vaccinations', val: dashboardStats.totalVaccinations,     page: 'vaccinations' },
          { icon: <><rect x="3" y="4" width="18" height="18" rx="2" ry="2" /><line x1="16" y1="2" x2="16" y2="6" /><line x1="8" y1="2" x2="8" y2="6" /><line x1="3" y1="10" x2="21" y2="10" /></>, label: 'Upcoming Appt.',     val: dashboardStats.rappelsProchains,      page: 'rappels' },
          { icon: <><circle cx="12" cy="12" r="10" /><line x1="12" y1="8" x2="12" y2="12" /><line x1="12" y1="16" x2="12.01" y2="16" /></>,                 label: 'Overdue Alerts',    val: dashboardStats.vaccinationsEnRetard, page: 'rappels', danger: true },
        ].map((c, i) => (
          <div key={i} className="stat-card" style={{ padding: '16px', position: 'relative', overflow: 'hidden', border: 'none', boxShadow: '0 4px 20px rgba(0,0,0,0.02)', cursor: 'pointer' }} onClick={() => setPage(c.page)}>
            ...
            <div className="stat-value" style={{ color: c.danger && dashboardStats.vaccinationsEnRetard > 0 ? COLORS.red : undefined }}>{c.val}</div>
