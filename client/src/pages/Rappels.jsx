...
export default function Rappels() {
  ...
  const enRetard = rappels.filter((rappel) => rappel.enRetard);
  const urgents = rappels.filter((rappel) => rappel.urgent && !rappel.enRetard);
  const prochains = rappels.filter((rappel) => !rappel.urgent && !rappel.enRetard);

  const getDelayLabel = (rappel) => {
    if (rappel.enRetard) {
      return langue === 'en'
        ? `${Math.abs(rappel.joursRestants)} days overdue`
        : `${Math.abs(rappel.joursRestants)} jours de retard`;
    }

    if (rappel.joursRestants === 0) {
      return t('rap_today');
    }

    return langue === 'en'
      ? `In ${rappel.joursRestants} days`
      : `Dans ${rappel.joursRestants} jours`;
  };
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
            <thead>
              <tr style={{ color: '#8a94a6', borderBottom: '1px solid #eaebef' }}>
                <th style={{ padding: '12px 8px', fontWeight: 600, fontSize: 12, textTransform: 'uppercase' }}>{t('rap_col_patient')}</th>
                <th style={{ padding: '12px 8px', fontWeight: 600, fontSize: 12, textTransform: 'uppercase' }}>{t('rap_col_contact')}</th>
                <th style={{ padding: '12px 8px', fontWeight: 600, fontSize: 12, textTransform: 'uppercase' }}>{t('rap_col_vaccin')}</th>
                <th style={{ padding: '12px 8px', fontWeight: 600, fontSize: 12, textTransform: 'uppercase' }}>{t('rap_col_date')}</th>
                <th style={{ padding: '12px 8px', fontWeight: 600, fontSize: 12, textTransform: 'uppercase' }}>{t('rap_col_action')}</th>
              </tr>
            </thead>
            <tbody>
              {items.map((rappel, index) => (
                <tr key={rappel.id} style={{ borderBottom: index < items.length - 1 ? '1px solid #eaebef' : 'none' }}>
                  <td style={{ padding: '16px 8px', display: 'flex', alignItems: 'center', gap: 12 }}>
                    <div style={{ width: 36, height: 36, borderRadius: '50%', background: '#f4f5f9', color: '#1d2129', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', fontSize: 13 }}>
                      {rappel.patient?.split(' ').map((name) => name[0]).join('').slice(0, 2).toUpperCase()}
                    </div>
                    <span style={{ fontWeight: 700, fontSize: 14, color: '#1d2129' }}>{rappel.patient}</span>
                  </td>

                  <td style={{ padding: '16px 8px' }}>
                    <div style={{ color: '#1d2129', fontSize: 13, fontWeight: 600 }}>{rappel.telephone || (langue === 'en' ? 'Not provided' : 'Non renseigne')}</div>
                  </td>

                  <td style={{ padding: '16px 8px' }}>
                    <div style={{ fontWeight: 800, color: theme.color, fontSize: 14 }}>{rappel.vaccin}</div>
                    <div style={{ fontSize: 12, color: '#8a94a6', marginTop: 2 }}>{rappel.dose}</div>
                  </td>

                  <td style={{ padding: '16px 8px', fontWeight: 600, fontSize: 13, color: '#1d2129' }}>
                    {new Date(rappel.dateProchaineDose).toLocaleDateString(locale)}
                  </td>

                  <td style={{ padding: '16px 8px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                      <span style={{ 
                        background: rappel.enRetard ? '#fee2e2' : rappel.joursRestants === 0 ? '#ffedd5' : theme.bg, 
                        color: rappel.enRetard ? '#ef4444' : rappel.joursRestants === 0 ? '#f97316' : theme.color, 
                        padding: '4px 10px', borderRadius: '6px', fontSize: 12, fontWeight: 700 
                      }}>
                        {getDelayLabel(rappel)}
                      </span>
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '32px' }}>
        <div>
          <h1 style={{ fontFamily: 'Syne', fontSize: 26, fontWeight: 800, color: '#1d2129', marginBottom: 4 }}>{t('rap_title_main')}</h1>
          <p style={{ color: '#8a94a6', fontSize: 14 }}>{t('rap_subtitle_main')}</p>
        </div>
        <div style={{ display: 'flex', gap: 12 }}>
          {enRetard.length > 0 && (
            <div style={{ background: '#fee2e2', color: '#ef4444', padding: '8px 16px', borderRadius: '50px', fontSize: 13, fontWeight: 800, display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              {enRetard.length} {t('rap_overdue').toLowerCase()}
            </div>
          )}
      ...
      {loading ? (
        <div style={{ textAlign: 'center', padding: 60, color: '#8a94a6' }}>{t('rap_loading')}</div>
      ) : rappels.length === 0 ? (
         <div style={{ textAlign: 'center', padding: '80px 20px', color: '#8a94a6', background: 'white', borderRadius: '16px', border: '1px dashed #eaebef' }}>
            ...
            <div style={{ fontSize: 18, fontWeight: 800, color: '#1d2129', marginBottom: 8, fontFamily: 'Syne' }}>{t('rap_all_good')}</div>
            <p style={{ margin: 0 }}>{t('rap_no_soon')}</p>
         </div>
      ) : (
        <>
          {enRetard.length > 0 && (
            <Section
              title={t('rap_sec_late')}
              items={enRetard}
              emptyMsg={t('rap_sec_late_empty')}
              theme={{ 
          ...
          {urgents.length > 0 && (
            <Section
              title={t('rap_sec_urgent')}
              items={urgents}
              emptyMsg={t('rap_sec_urgent_empty')}
              theme={{ 
          ...
          <Section
            title={t('rap_sec_plan')}
            items={prochains}
            emptyMsg={t('rap_sec_plan_empty')}
            theme={{ 
