// server.js — VacciTrack avec MySQL via un adaptateur synchrone
// ═══════════════════════════════════════════════════════════════════════════════
// STRUCTURE :
//   1. Imports & configuration
//   2. Multer (upload Excel)
//   3. Helpers (normalisation, dates, texte)
//   4. Lecture Excel (PowerShell / xlsx)
//   5. Import en base (ANTIRAB brut / workbook exporté)
//   6. Routes : PATIENTS
//   7. Routes : VACCINATIONS
//   8. Routes : ORDONNANCES
//   9. Routes : STOCKS
//  10. Routes : RAPPELS
//  11. Routes : STATS
//  12. Routes : SETTINGS
//  13. Routes : SUPPORT TICKETS
//  14. Routes : HELP ARTICLES
//  15. Routes : MAP
//  16. Routes : IMPORT ANTIRAB
//  17. Routes : DIVERS
//  18. PDF — helpers & builders (vaccination, ordonnance, DT)
//  19. Routes PDF
//  20. app.listen
// ═══════════════════════════════════════════════════════════════════════════════
...
const { sendTicketNotification, checkSmtpConfig } = require('./Emailservice');
...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    allowed.includes(ext) ? cb(null, true) : cb(new Error('Seuls les fichiers .xlsx et .xls sont acceptés'));
  },
...
const normalizeHeader = value =>
  normalizeKey(value).normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-z0-9]/g, '');
...
const parseExcelDate = (value) => {
  ...
  if (compactDigits) {
    ...
    return buildIsoDate(digits.slice(0, 4), digits.slice(4, 6), digits.slice(6, 8))
        || buildIsoDate(digits.slice(4, 8), digits.slice(2, 4), digits.slice(0, 2));
  }

  const dayMonthYear = raw.match(/^(\d{1,2})\/(\d{1,2})\/(\d{2,4})$/);
  if (dayMonthYear) {
    const [, d, m, y] = dayMonthYear;
    return buildIsoDate(y.length === 2 ? `20${y}` : y, m, d);
  }
...
const extractPatientIdentity = (row) => {
  ...
  return { nom: normalizeText(nom), prenom: normalizeText(prenom) };
};

// ── 4. Lecture Excel ──────────────────────────────────────────────────────────

/** Lit un fichier Excel via PowerShell (Windows). */
function readAntirabWorkbook(workbookPath) {
  return new Promise((resolve, reject) => {
    ...
    execFile(
      'powershell',
      ['-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', scriptPath, workbookPath],
      ...
      (err, stdout, stderr) => {
        ...
        try {
          ...
          reject(new Error(`Lecture Excel invalide : ${parseErr.message}`));
        }
      }
    );
  });
...
/** Lit un fichier Excel via la lib xlsx (toutes plateformes). */
  ...
  try {
    ...
    return {
      ...
      sheets: workbook.SheetNames.map(name => ({
        name,
        rows: XLSX.utils.sheet_to_json(workbook.Sheets[name], { defval: '', raw: true }),
      })),
    };
  } catch (err) {
    throw new Error(`Impossible de lire le fichier Excel (xlsx) : ${err.message}`);
...
/** Détecte si le classeur est un export VacciTrack (onglets 'patients' + 'vaccinations'). */
function isExportedWorkbook(workbook) {
  const names = new Set((workbook?.sheetNames || []).map(n => normalizeKey(n)));
  return names.has('patients') && names.has('vaccinations');
}

// ── 5. Import en base ─────────────────────────────────────────────────────────

/** Importe un workbook exporté depuis VacciTrack (onglets structurés). */
function importExportedWorkbookIntoDB(workbook) {
  const patientsSheet     = (workbook.sheets || []).find(s => normalizeKey(s.name) === 'patients');
  const vaccinationsSheet = (workbook.sheets || []).find(s => normalizeKey(s.name) === 'vaccinations');
  ...
  const stmts = {
    findById:       db.prepare('SELECT id FROM patients WHERE id = ? LIMIT 1'),
    findByIdentity: db.prepare(`
    ...
    insertPatient: db.prepare(`
      ...
      VALUES
        (@id,@nom,@prenom,@dateNaissance,@age,@sexe,@telephone,@email,@adresse,@wilaya,@daira,
         ...
         @fumeur,@alcool,@activitePhysique,@mutuelle,@numeroCNAS,@medecinTraitant,@notesClinicien,@createdAt)
    `),
    findVaccinById: db.prepare('SELECT id FROM vaccinations WHERE id = ? LIMIT 1'),
    findVaccinBySig: db.prepare(`
  ...
  const result     = { totalRows: patientRows.length + vaccinationRows.length, importedPatients: 0, importedVaccinations: 0, skippedRows: 0 };
  const idMap      = new Map();
  ...
  db.transaction(() => {
    ...
    for (const row of patientRows) {
      ...
      if (!nom || !prenom) { result.skippedRows++; continue; }

      let patient = sourceId ? stmts.findById.get(sourceId) : null;
      if (!patient) patient = stmts.findByIdentity.get(normalizeKey(nom), normalizeKey(prenom), sexe);

      if (!patient) {
        let patientId = sourceId || uuidv4();
        if (sourceId && stmts.findById.get(sourceId)) patientId = uuidv4();
        stmts.insertPatient.run({
          ...
          age:                row.age === '' || row.age == null ? null : Number.parseInt(row.age, 10) || null,
          sexe,
      ...
      if (sourceId) idMap.set(sourceId, patient.id);
    }

    // --- Vaccinations ---
    for (const row of vaccinationRows) {
      const sourceVaccinId  = normalizeText(row.id);
      const sourcePatientId = normalizeText(row.patientId);
      let   patientId       = sourcePatientId ? (idMap.get(sourcePatientId) || sourcePatientId) : '';
      ...
      if (!patientId && normalizeText(row.patient)) {
        ...
        const found  = (nom || prenom)
          ? stmts.findByIdentity.get(normalizeKey(nom), normalizeKey(prenom), normalizeText(row.sexe) || 'M')
          : null;
        if (found) patientId = found.id;
      }

      if (!patientId || !stmts.findById.get(patientId)) { result.skippedRows++; continue; }
      if (sourceVaccinId && stmts.findVaccinById.get(sourceVaccinId)) { result.skippedRows++; continue; }
      ...
      if (stmts.findVaccinBySig.get(patientId, type, vaccin, dateAdministration || '', dose)) {
        result.skippedRows++; continue;
      }
      ...
      if (!protocoleData) {
        protocoleData = { grade: normalizeText(row.grade) || null, schema: normalizeText(row.schema) || null };
      }

      let vaccinationId = sourceVaccinId || uuidv4();
      if (sourceVaccinId && stmts.findVaccinById.get(sourceVaccinId)) vaccinationId = uuidv4();

      stmts.insertVaccin.run({
...
/** Importe des lignes brutes au format ANTIRAB. */
function importRowsIntoDB(rows) {
  const stmts = {
    findPatient: db.prepare(`
      SELECT id FROM patients
      WHERE lower(trim(nom)) = ? AND lower(trim(prenom)) = ? AND coalesce(sexe,'') = ?
      LIMIT 1
    `),
  ...
  db.transaction((items) => {
    for (const row of items) {
      ...
      let patient = stmts.findPatient.get(normalizeKey(nom), normalizeKey(prenom), sexe);
      if (!patient) {
        const patientId = uuidv4();
        stmts.insertPatient.run({
          id: patientId, nom, prenom, dateNaissance,
          ...
          mutuelle: null, numeroCNAS: null, medecinTraitant: null, notesClinicien: null,
      ...
      if (stmts.findVaccin.get(patient.id, dateAdministration, vaccin)) { result.skippedRows++; continue; }
      ...
      stmts.insertVaccin.run({
        id: uuidv4(), patientId: patient.id,
        type: 'rage', vaccin,
        dose:              normalizeText(pickRowValue(row, ['VARDOSE'])) || null,
        statut:            'complete',
        dateAdministration,
        dateProchaineDose: null,
        protocoleData:     JSON.stringify(protocoleData),
        createdAt:         dateAdministration,
      });
      result.importedVaccinations++;
...
app.get('/api/patients', (req, res) => {
  try {
    ...
    const sql      = (all === '1' || search || year)
      ? `SELECT * FROM patients${whereSql} ORDER BY createdAt DESC`
...
app.get('/api/export/patients', (req, res) => {
  try {
    ...
    const vaccins  = db.prepare('SELECT * FROM vaccinations ORDER BY dateAdministration DESC, createdAt DESC').all().map(hydrateVacc);

    const patientNames = patients.reduce((acc, p) => {
      acc[p.id] = `${p.prenom || ''} ${p.nom || ''}`.trim();
      return acc;
    }, {});
    ...
    const summaryRows = [
      ...
      { champ: 'vaccinations', valeur: vaccins.length },
    ];

    const patientsVaccinsRows = patients.flatMap(p => {
      const pv = vaccins.filter(v => v.patientId === p.id);
      if (!pv.length) return [{
        patientId: p.id, nom: p.nom||'', prenom: p.prenom||'', dateNaissance: p.dateNaissance||'',
        age: p.age??'', sexe: p.sexe||'', telephone: p.telephone||'', commune: p.commune||'', wilaya: p.wilaya||'',
        typeVaccin: '', vaccin: '', dose: '', statut: '', dateAdministration: '', dateProchaineDose: '', grade: '', schema: '',
      }];
      return pv.map(v => ({
        patientId: p.id, nom: p.nom||'', prenom: p.prenom||'', dateNaissance: p.dateNaissance||'',
        age: p.age??'', sexe: p.sexe||'', telephone: p.telephone||'', commune: p.commune||'', wilaya: p.wilaya||'',
        typeVaccin: v.type||'', vaccin: v.vaccin||'', dose: v.dose||'', statut: v.statut||'',
        dateAdministration: v.dateAdministration||'', dateProchaineDose: v.dateProchaineDose||'',
        grade: v.protocoleData?.grade||'', schema: v.protocoleData?.schema||'',
      }));
    });

    const patientRows = patients.map(p => ({
      id: p.id, nom: p.nom||'', prenom: p.prenom||'', dateNaissance: p.dateNaissance||'',
      age: p.age??'', sexe: p.sexe||'', telephone: p.telephone||'', adresse: p.adresse||'',
      adressePrecise: p.adressePrecise||'', commune: p.commune||'', daira: p.daira||'', wilaya: p.wilaya||'',
      profession: p.profession||'', fonction: p.fonction||'', groupeSanguin: p.groupeSanguin||'',
      createdAt: p.createdAt||'', vaccinationsCount: vaccins.filter(v => v.patientId === p.id).length,
    }));

    const vaccinRows = vaccins.map(v => ({
      id: v.id, patientId: v.patientId||'',
      patient: v.patient || patientNames[v.patientId] || '',
      type: v.type||'', vaccin: v.vaccin||'', dose: v.dose||'', statut: v.statut||'',
      dateAdministration: v.dateAdministration||'', dateProchaineDose: v.dateProchaineDose||'',
      grade: v.protocoleData?.grade||'', schema: v.protocoleData?.schema||'',
      protocoleData: v.protocoleData ? JSON.stringify(v.protocoleData) : '',
      createdAt: v.createdAt||'',
    }));

    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(patientsVaccinsRows), 'Patients_Vaccins');
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(patientRows),         'Patients');
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(vaccinRows),           'Vaccinations');
...
app.post('/api/patients', (req, res) => {
  try {
    ...
    `).run({
      ...
      fumeur:        p.fumeur || null, alcool: p.alcool || null,
...
app.put('/api/patients/:id', (req, res) => {
  try {
    ...
    `).run({
      ...
      age:           Number.isFinite(Number(b.age)) ? Number(b.age) : null,
      sexe:          b.sexe || 'M',
      telephone:     b.telephone || null, email:     b.email || null,
      adresse:       b.adresse   || null, wilaya:    b.wilaya || null,
      daira:         b.daira     || null, commune:   b.commune || null,
      adressePrecise: b.adressePrecise || null, groupeSanguin: b.groupeSanguin || 'A+',
      poids:         String(b.poids || ''),
      fonction:      b.fonction   || null, service:   b.service || null,
      profession:    b.profession || null, instruction: b.instruction || null,
      antecedents:   b.antecedents || null, allergies: b.allergies || null,
      maladiesChroniques: b.maladiesChroniques || null,
      traitementEnCours:  b.traitementEnCours  || null,
      contreIndications:  b.contreIndications  || null,
      fumeur:        b.fumeur || null, alcool: b.alcool || null,
      activitePhysique: b.activitePhysique || null,
...
app.get('/api/vaccinations', (req, res) => {
  try {
    ...
    res.json(db.prepare(`${join}${where} ORDER BY v.dateAdministration DESC`).all(...params).map(hydrateVacc));
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.post('/api/vaccinations', (req, res) => {
  try {
    ...
    `).run({
      ...
      dateAdministration: v.dateAdministration || null, dateProchaineDose: v.dateProchaineDose || null,
      protocoleData: v.protocoleData ? JSON.stringify(v.protocoleData) : null,
      createdAt: v.createdAt,
    });

    // Décrémenter le stock correspondant
    ...
    if (match) {
      ...
      db.prepare(`INSERT INTO stock_movements (id,stockId,type,quantite,patientId,motif,createdAt) VALUES (?,?,'SORTIE',1,?,?,?)`)
...
app.post('/api/stocks', (req, res) => {
  try {
    ...
    db.prepare(`INSERT INTO stock_movements (id,stockId,type,quantite,motif,createdAt) VALUES (?,?,'ENTREE',?,?,?)`)
      .run(uuidv4(), s.id, Number(s.quantiteInitiale) || 0, 'Réception initiale de lot', new Date().toISOString());
...
app.put('/api/stocks/:id', (req, res) => {
  try {
    const b = req.body;
    db.prepare(`
      UPDATE stocks SET vaccin=@vaccin, lot=@lot, quantiteInitiale=@quantiteInitiale,
        quantiteRestante=@quantiteRestante, datePeremption=@datePeremption WHERE id=@id
    `).run({
  ...
  try {
    ...
    const parAge             = db.prepare(`
      SELECT CASE
        ...
        ELSE '60+' END as tranche, COUNT(*) as count
      FROM patients ${pSQL} GROUP BY tranche
    `).all(...pP);
...
const createTicket = async (req, res) => {
  try {
    ...
    sendTicketNotification(ticket).catch(err =>
      console.error('Erreur envoi email ticket :', err.message)
    );
  } catch (err) { res.status(500).json({ error: err.message }); }
};
...
const getHelpArticles = (req, res) => {
  try {
    ...
    const where = clauses.length ? ` WHERE ${clauses.join(' AND ')}` : '';
    res.json(db.prepare(`SELECT * FROM help_articles${where} ORDER BY createdAt DESC, titre ASC`).all(...params));
  } catch (err) { res.status(500).json({ error: err.message }); }
};
...
const postHelpFeedback = (req, res) => {
  try {
    ...
    db.prepare(`INSERT INTO help_feedback (id,articleId,feedbackType,createdAt) VALUES (?,?,?,?)`)
...
app.get('/api/map/communes', (req, res) => {
  try {
    const rows = db.prepare(`
      SELECT
        COALESCE(NULLIF(trim(commune),''), 'Non spécifiée') as commune,
        COUNT(*) as count,
        SUM(CASE WHEN sexe = 'M' THEN 1 ELSE 0 END) as hommes,
        SUM(CASE WHEN sexe = 'F' THEN 1 ELSE 0 END) as femmes
      FROM patients
      GROUP BY COALESCE(NULLIF(trim(commune),''), 'Non spécifiée')
...
app.post('/api/import-antirab', async (req, res) => {
  try {
    ...
    res.json({ message: 'Import ANTIRAB terminé.', ...importRowsIntoDB(rows) });
  } catch (err) {
  ...
  try {
    ...
    if (isExportedWorkbook(workbook)) {
      ...
      try   { rows = await readAntirabWorkbook(filePath); }
        ...
        console.log(`PowerShell indisponible (${psErr.message}), utilisation de xlsx…`);
        rows = await readExcelWithXlsx(filePath);
      }
      if (!rows || !rows.length) return res.status(400).json({ error: 'Le fichier est vide ou illisible.' });
      result = importRowsIntoDB(rows);
    }

    console.log(`✅ Import terminé : ${result.importedPatients} patients, ${result.importedVaccinations} vaccinations`);
    res.json({ message: 'Import terminé avec succès.', ...result });
  } catch (err) {
...
app.get('/api/vaccins-disponibles', (req, res) => {
  res.json([
    'Anti-Rabique Tissulaire', 'Anti-Rabique Cellulaire',
    'Hépatite B', 'DT', 'BCG', 'ROR', 'Grippe', 'COVID-19',
  ]);
});
...
/** Supprime les emoji et caractères non supportés par pdf-lib. */
...
// Coordonnées ordonnance (origine bas-gauche pdf-lib)
...
// Coordonnées carte DT (carte_de_vaccination_dt.pdf — A4 paysage)
const DT_COORDS = {
  p1: {
...
/** Génère la carte PDF Diphtérie-Tétanos. */
async function buildDtPdf(payload) {
  console.log('🔍 DT PDF — Payload reçu :', JSON.stringify(payload, null, 2));
  const pdfDoc = await PDFDocument.load(fs.readFileSync(path.join(__dirname, 'carte_de_vaccination_dt.pdf')));
  ...
  const draw = (pg, text, coord, isBold = false) => {
    ...
    if (coord.maxW) while (t.length > 1 && f.widthOfTextAtSize(t, coord.size) > coord.maxW) t = t.slice(0, -1);
  ...
  console.log('🔍 DT PDF — Dates :', { d1Raw, d2, d3, rappel10 });
  ...
  draw(p1, [patient.adressePrecise, patient.adresse, patient.commune, patient.wilaya].filter(Boolean).join(', '), DT_COORDS.p1.adresse);
  ...
  if (medecinDT) {
    draw(p2, /^Dr\.?/i.test(medecinDT) ? medecinDT : `Dr. ${medecinDT}`, DT_COORDS.p2.medecin, true);
  }
...
/** Génère la carte PDF vaccination (anti-rabique et autres). */
async function buildVaccinationPdf(payload) {
  ...
  const pdfDoc = await PDFDocument.load(fs.readFileSync(path.join(__dirname, 'carte_de_vaccination_z.pdf')));
  const font   = await pdfDoc.embedFont(StandardFonts.Helvetica);
  const bold   = await pdfDoc.embedFont(StandardFonts.HelveticaBold);
  const pages  = pdfDoc.getPages();
  const p1     = pages[0];
  ...
  if (type === 'rage') {
    ...
    const colX      = (proto.grade === 'Grade III') ? 183 : 97;
    const labelToRow = {
      'J0':'J0', 'J3':'J3', 'J7':'J5', 'J14':'J14',
      'J28':'J29', 'J30':'J29', 'J90':'J90', 'J29':'J29', 'J34':'J34',
    };
    for (const [label, dateVal] of Object.entries(datesVAR)) {
      const rowKey = labelToRow[label] || label;
      const rowDef = TABLE_ROWS[rowKey];
      if (rowDef && dateVal) drawDate(p1, dateVal, colX, rowDef.y, 7.5, 80);
    }
  }
  ...
  else if (espece === 'autre') {
    dot(p2, 180, 330, 4);
    draw(p2, proto.especeAnimalePrecise || '', 228, 326, 8, false, 65);
  }
  ...
  if (proto.erig) {
    ...
    if (proto.suture) {
      dot(p2, 110, 245, 4);
      if (proto.sutureDetail?.toLowerCase().includes('avant')) dot(p2, 130, 235, 4);
  ...
  if (type === 'rage' && proto.varType === 'cellulaire') {
    ...
    if (protocole === 'ZAGREB') {
      const map = { 'J0 (2 sites)':{x:394,y:300}, 'J7':{x:394,y:336}, 'J21':{x:394,y:304}, 'Rappel (J90)':{x:394,y:280} };
      for (const [label, dateVal] of Object.entries(datesVAR)) {
        const coord = map[label];
        if (coord && dateVal) drawDate(p2, dateVal, coord.x, coord.y, 7.5, 85);
      }
    } else {
      const map = { 'J0':{x:488,y:300}, 'J3':{x:488,y:354}, 'J7':{x:488,y:336}, 'J14':{x:488,y:320}, 'J28':{x:488,y:304} };
      for (const [label, dateVal] of Object.entries(datesVAR)) {
        const coord = map[label];
        if (coord && dateVal) drawDate(p2, dateVal, coord.x, coord.y, 7.5, 85);
      }
  ...
  // Vaccins non-rage (grippe, hepb, pneumo, meningo, etc.)
  if (type !== 'rage') {
    const lines = [];
    if      (type === 'grippe')   lines.push(`Grippe ${proto.saison||''} — ${proto.souche||''} | ${proto.marque||''} | Lot : ${proto.lot||'-'}`);
    else if (type === 'hepb')     lines.push(`Hépatite B — Schéma : ${proto.schema||'-'} | ${proto.marque||''} | Lot : ${proto.lot||'-'}`);
    else if (type === 'dt')       lines.push(`DT — Schéma : ${proto.schema||'-'} | ${proto.marque||''} | Lot : ${proto.lot||'-'}`);
    else if (type === 'pneumo')   lines.push(`Pneumocoque — ${proto.typeVaccin||''} | ${proto.numeroDose||'-'} | Lot : ${proto.lot||'-'}`);
    else if (type === 'meningo')  lines.push(`Méningocoque — ${proto.typeVaccin||''} | ${proto.numeroDose||'-'} | Lot : ${proto.lot||'-'}`);
...
async function buildOrdonnancePdf(payload) {
  const pdfDoc = await PDFDocument.load(fs.readFileSync(path.join(__dirname, 'ordonnance_haute_qualite.pdf')));
  const font   = await pdfDoc.embedFont(StandardFonts.Helvetica);
  const bold   = await pdfDoc.embedFont(StandardFonts.HelveticaBold);
  ...
  const rawAge    = normalizeText(patient.age);
  const patientAge = rawAge
    ? (/\bans?\b/i.test(rawAge) ? rawAge : `${rawAge} ans`)
...
const server = app.listen(PORT, () => {
  ...
  console.log(`📂 Import Excel : POST /api/import-antirab-upload`);
  checkSmtpConfig(); // Vérifie et affiche le statut SMTP au démarrage
});

server.on('error', (err) => {
  if (err?.code === 'EADDRINUSE') {
    console.error(`❌ Le port ${PORT} est déjà utilisé. Stoppe l'autre instance ou change PORT dans server/.env.`);
    if (process.env.VACCITRACK_ALLOW_PORT_CONFLICT === '1') {
      console.error('ℹ️ Lancement Electron: le serveur local n’a pas pu reprendre le port, mais l’application continue.');
      return;
    }
    process.exit(1);
  }
